...
**尝试将Rws直接固定，   rmse	0.366709 **  （没有合并线特征）
...
MH_05_折腾事件  Rws是一直固定住的

1. 限制优化的最大求解时间为60ms，迭代次数最多9次，        rmse	0.185021  ——并未变差太多
2. 待优化的点线特征的最少观测次数为**都为6次**        rmse	0.344125
3. 点4 线6    rmse	0.179147
4. max_solver_time: 0.05    rmse	0.195836
5. 定义的Tictoc类型的数据不要读两次
6. 限制了优化时间后，电脑的运行状态就会影响运行结果  刚开机，即使限制最大优化时间50ms，  仍然可以达到    rmse	0.178580
7. 将合并线特征的函数注释掉，发现结果变化不大   rmse	0.180334     平均优化时间71ms
8. 去掉了仅优化线，结果变差，       rmse	0.271037   （仍然限制优化时间） 平均优化时间        rmse	0.271037， 即使不限制优化时间，效果仍然不咋滴
9. 最新开机，平均计算事件68ms，remse0.16多

### 改成03的数据直接运行         rmse	0.148027  72ms

按照以前的，调整好所有的参数        rmse	0.124699



### 用MH_03的程序直接跑MH_04      rmse	0.194923   平均优化时间64ms

加上合并线特征---只要三角化就合并    rmse	0.191320  

改为仅优化后的线就参加合并      rmse	0.254889   



### 之前写的MH_02的程序，限制了优化时间，原本0.16,现在却变成0.14 平均优化时间  72.213152
