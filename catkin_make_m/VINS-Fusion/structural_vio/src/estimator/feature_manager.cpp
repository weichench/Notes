...
{ 
 for(auto &it_per_id : linefeature)
 {
   ...
   if(line_on_point.count(it_per_id.feature_id) == 0 )
     continue;
   vector<int> id_point = line_on_point[it_per_id.feature_id];
   Vector3d w_pts_i;
   
   //找到在线上的，已经三角化了的点，得其在世界系下的坐标
   for(auto &it_per_point : id_point)
   {
    auto it = find_if(feature.begin(), feature.end(), [it_per_point](const FeaturePerId &it)
                          {
            return it.feature_id == it_per_point;
                          });

        if (it == feature.end())
           continue;
	
        else if (it->feature_id == it_per_point)
        {
	  //表示找到了那个在线上的点
...
// 	    Vector3d pt2_l = it_per_id.Rsl.transpose()*it_per_id.pts2;
	    ...
	    Vector3d ptl1, ptl2, ptw1, ptw2, ptc1, ptc2;
	    
	    ptl1 = ptm_l;
	    ptl2 = ptm_l + Vector3d(0,0,1);
	    
	   ptw1 = Rws.back()*it_per_id.Rsl*ptl1;
	   ptw2 = Rws.back()*it_per_id.Rsl*ptl1;
  ...
  if(it_per_id.is_triangulation)
  {
   ...
   {//该帧的Rcs有问题，不在该帧三角化
	...
	if(pts_1(2) >=0 && pts_2(2) >=0 &&
	  line_length > 0.25  /*&& line_origin_distance(pts_1, pts_2) < 15*/ &&
	  pts_1.norm() < 30 && pts_2.norm() < 30 )
	{  //越远的点，误差是越大的
