...
void Estimator::setParameter()
{
    ...
    ProjectionStructuralLine::sqrt_info = FOCAL_LENGTH / 1.2 * Matrix2d::Identity();
...
//     float cy = 2.401e+02;
    ...
    float fx = 368.793999752765;
    float fy = 368.3670250066841;
    float cx = 3.760e+02;
    float cy = 2.401e+02;
			     ...
			     const pair<double, vector<cv::Point3d>> &vps)
{
   ...
   {//加入开头，一定时间后，再使用线特征
         if( header > 1403638155  && !if_use_line)
    {
...
//     会得到Rcs[frame_count] and Rws_state[frame_count]
    ...
    if(1)
  {
     ...
     cv::Mat image_show_point, image_show_right_point;
      featureTracker.cur_img.copyTo(image_show);
      featureTracker.rightImg.copyTo(image_show_right);
      featureTracker.cur_img.copyTo(image_show_point);
      featureTracker.rightImg.copyTo(image_show_right_point);
...
// 	    cv::putText(image_show_right, write_pt2_right, pt2, cv::FONT_HERSHEY_DUPLEX, 0.5, cv::Scalar(0, 255,0 ), 1);
	    ...
	    cv::line(image_show_right, pt1, pt2,color, 2);
	    }
	  }
	}
// 	cout<<endl;
...
//      
	...
	if(i == 0)
      ...
      {
	Vector4d left_ob = id_line.second[0].second;
	
	  cv::Point2d pt1, pt2, vp;
	    ...
	    vp = To_picture(vp);
	     ...
	     cv::line(image_show, pt1, pt2,color, 2);
	     cv::line(image_show, pt1, vp,color, 1);
...
//         cv::line(image_show, pt1, pt2,cv::Scalar(0,0,255), 2);
	...
	vector<int> id_points = it_per_line.second;
	
	for(auto &it_per_point : id_points)
	{
	   vector<pair<int, Eigen::Matrix<double, 7, 1>>> point_info = points[it_per_point];
	   Eigen::Matrix<double, 7, 1> point_observe = point_info[0].second;
	    ...
	    pt3.y = point_observe(1);
	     ...
	     cv::circle(image_show,pt3,5,CV_RGB(255,255,0),1);
	     cv::circle(image_show,pt3,3,CV_RGB(255,255,0),-1);
//              cv::line(image_show, pt1, pt3,cv::Scalar(0,255,0), 1);
...
//  	      f_manager.triangulateLine_from_point(Ps, tic, ric, Rws, Which_Rws, Rws_state, line_on_point);
	      f_manager.triangulateLine(Ps, tic, ric, Rws, Which_Rws, Rws_state);
	      //线的三角化
	    }
...
/*	  
    ...
    if(USE_IMU)
    {
	 ...
	 cout<<"refined tic"<<i<<": "<<tic[i].transpose()<<endl;
	 cout<<"refined ric"<<i<<"\n"<<ric[i]<<endl;
        }
    }
...
// 	    vector<pair<int, Eigen::Matrix<double, 7, 1>>> point_info = points[it_per_id];
	...
	{ 
	  ...
	  double angle_cos = p_lp1.dot(p_lp2)/(p_lp1.norm()*p_lp2.norm());
	   ...
	   if(angle_cos < 0)
	    line_on_point[id_line.first].emplace_back(it_point);
	   
	   else
	   {
	     double length = min(p_lp1.norm(), p_lp2.norm());
	       if(length < 100.0/FOCAL_LENGTH)
	       { line_on_point[id_line.first].emplace_back(it_point);
	       } 
	  }
