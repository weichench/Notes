...
//     Structural_lines.scale.x = 0.08;
//     Structural_lines.scale.y = 0.08;
//     
//     Structural_lines.scale.z = 0.08;
//      Structural_lines.color.b = 1.0;
...
//             continue;
      ...
      if (!it_per_id.solve_flag || it_per_id.is_triangulation == false)
        continue;
...
//           Vector3d w_pts_2 =  estimator.Rs[base_frame] * (estimator.ric[0] * ptc_2 + estimator.tic[0]) + estimator.Ps[base_frame];
  ...
  if(estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
  {
    ...
    marg_lines_cloud.scale.x = 0.08;
    marg_lines_cloud.scale.y = 0.08;
    marg_lines_cloud.scale.z = 0.08;
    ...
    for (auto &it_per_id : estimator.f_manager.linefeature)
    {
      if (it_per_id.linefeature_per_frame.front().In_frame_count == 0 && it_per_id.linefeature_per_frame.size() <= 2
        ...
        {
	   if(it_per_id.max_num_observatoin < 8)
	    continue;
