...
void MSAC::init(int mode, cv::Mat K, cv::Size imSize, bool verbose)
{
	// Arguments
	__verbose = verbose;
...
// 	__T_noise_squared = (float)0.03;
// 	__T_noise_squared = (float)0.01623*2;
// 	__T_noise_squared = (float)0.02;
	__T_noise_squared = (float)0.1;     //best value in Euroc 0.015
	__min_iters = 5;
	__max_iters = 1000;
	...
	__K = Mat(3,3,CV_32F);
	__K.setTo(0);
// 	__K.at<float>(0,0) = 4.616e+02;
// 	__K.at<float>(0,2) = 3.630e+02;
// 	__K.at<float>(1,1) = 4.603e+02;
// 	__K.at<float>(1,2) = 2.481e+02;
// 	__K.at<float>(2,2) = (float)1;	
	
        __K.at<float>(0,0) = K.at<float>(0,0);
	__K.at<float>(0,2) = K.at<float>(0,2);
	__K.at<float>(1,1) = K.at<float>(1,1);
	__K.at<float>(1,2) = K.at<float>(1,2);
	__K.at<float>(2,2) = (float)1;	
...
void MSAC::multipleVPEstimation(std::vector<std::vector<cv::Point> > &lineSegments, 
				std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, 
				std::vector<int> &numInliers, std::vector<cv::Mat> &vps, int numVps, cv::Point2d vp_g)
{	
...
//            if(vpNum == 0)
// 	   {
// 	      __vpAux.at<float>(0,0) = vp_g.x;
// 	      __vpAux.at<float>(0,0) = vp_g.y;
// 	      __vpAux.at<float>(0,0) = 1;
// 	       cv::normalize(__vpAux, __vpAux);
// 	      
// 	       	int N_I = 0;//????????
// 			float J = GetConsensusSet(vpNum, __Li, __Lengths, __Mi, __vpAux, E, &N_I);
// 			
// 	    printf("The number of vertical lines %d \n", N_I);
// 	       
// 	   }
//        
//          else
	 {
		...
		{					
			...
			if(__Li.rows < (int)__MSS.size())
				break;
			do
			{GetMinimalSampleSet(__Li, __Lengths, __Mi, __MSS, __vpAux);
			  
			}while(__vpAux.at<float>(2,0)==0);
			
			printf("Random vanishing point is = (%.3f,%.3f,%.3f)\n", __vpAux.at<float>(0,0), __vpAux.at<float>(1,0), __vpAux.at<float>(2,0));				
	...
	__lineSegments_init.push_back(lineSegmentsCurrent);
	__vps_init.push_back(__vp);
...
// 		E[i] = di*di;
		...
		/* Add to CS if error is less than expected noise */
		double threshold_value = __T_noise_squared;
		
// 		if(vpNum == 0)
// 		 threshold_value = threshold_value*2;  
		  
		if (E[i] <= threshold_value)
		{
			__CS_idx[i] = vpNum;		// set index to 1
			(*CS_counter)++;
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	for(unsigned int vpNum=0; vpNum<vps.size(); vpNum++)
	{
		if(vps[vpNum].at<float>(2,0) != 0)
		{
			...
			{
				cv::circle(im, vp, 4, colors[vpNum], 2);	
				cv::circle(im, vp, 3, CV_RGB(0,0,0), -1);				
			}
		}
	}
	// Paint line segments
