...
private:
	...
	float __epsilon;
	float __P_inlier;
	float __T_noise_squared;      //判断是否为内点的阈值
	int __min_iters;
	int __max_iters;
	bool __reestimate;
	bool __verbose;
...
public:
	...
	void init(int mode, cv::Mat K,  cv::Size imSize, bool verbose=false);
	...
	void multipleVPEstimation(std::vector<std::vector<cv::Point> > &lineSegments, 
	...
	std::vector<std::vector<std::vector<cv::Point> > > __lineSegments_init;
	std::vector<cv::Mat> __vps_init;
	std::vector<std::vector<int>> __ind_cs;
