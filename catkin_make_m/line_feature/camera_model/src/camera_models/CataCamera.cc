...
CataCamera::Parameters::readFromYamlFile(const std::string& filename)
{
   ...
   std::cout<<"creat a mei camera"<<std::endl;
    return true;
}
...
CataCamera::spaceToPlane(const Eigen::Vector3d& P, Eigen::Vector2d& p) const
{
    ...
    {   /*std::cout<<"Apply a distortion"<<std::endl;*/
        // Apply distortion
        Eigen::Vector2d d_u;
        distortion(p_u, d_u);
        p_d = p_u + d_u;
                                    ...
                                    cv::Mat rmat) const
{
	...
	K_rect(0,2) = mParameters.u0();
	K_rect(1,2) = mParameters.v0();
    ...
    for (int v = 0; v < imageSize.height; ++v)
    {
        for (int u = 0; u < imageSize.width; ++u)
        {
            ...
            Eigen::Vector3d uo = R_inv.cast<double>() * K_rect_inv.cast<double>() * xo;
            //Cp系下的归一化平面的点
            Eigen::Vector2d p;
	    Eigen::Vector3d p_cp;
	    
	 double mx_u = uo(0), my_u = uo(1);
	if(1)
        { //归一化平面的去畸变
            // Recursive distortion model
            int n = 8;
            Eigen::Vector2d d_u;
            distortion(Eigen::Vector2d(uo(0), uo(1)), d_u);
            // Approximate value
            mx_u = uo(0) + d_u(0);
            my_u = uo(1) + d_u(1);
...
//             }
    ...
    // Obtain a projective ray  相机坐标系下的点
    double xi = mParameters.xi();
    if (xi == 1.0)
    {
        p_cp << mx_u, my_u, (1.0 - mx_u * mx_u - my_u * my_u) / 2.0;
    }
    else
    {
        // Reuse variable
        double rho2_d = mx_u * mx_u + my_u * my_u;
        p_cp << mx_u, my_u, 1.0 - xi * (rho2_d + 1.0) / (xi + sqrt(1.0 + (1.0 - xi * xi) * rho2_d));
    }
 ...
 {
    Eigen::Vector2d p_u, p_d;
    double z = p_cp(2) + mParameters.xi() * p_cp.norm();
    p_u << p_cp(0) / z, p_cp(1) / z;
